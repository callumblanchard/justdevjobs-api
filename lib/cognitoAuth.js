...
function verifyProm(pems, auth) {
  return new Promise((resolve, reject) => {
    ...
    (err, decodedAndVerified) => {
      ...
      // The signature matches so we know the JWT token came from our Cognito instance, now just
      // verify the remaining claims in the token
      ...
      // Verify the client id matches what we expect. Will be in either the aud or the client_id
      // claim depending on whether it's an id or access token.
...
function verifyMiddleWare(pemsDownloadProm, req, res, next) {
  pemsDownloadProm.then((pems) => verifyProm(pems, req.get('Authorization')))
    .then((decoded) => {
...
function getVerifyMiddleware() {
  ...
  return (req, res, next) => {
    verifyMiddleWare(pemsDownloadProm, req, res, next);
  };
}
